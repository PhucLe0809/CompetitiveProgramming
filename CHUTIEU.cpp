...
Bint f[maxarray], t[maxarray], g[maxarray];

void init(){
    f[1] = Bint(1); f[2] = Bint(2);
    for (int i = 3; i < maxarray; i++) f[i] = f[i-1] + f[i-2];
    t[1] = Bint(1); t[2] = Bint(2); t[3] = Bint(4);
    for (int i = 4; i < maxarray; i++) t[i] = t[i-1] + t[i-2] + t[i-3];
    g[1] = Bint(0); g[2] = Bint(0); g[3] = Bint(1);
    for (int i = 4; i < maxarray; i++) g[i] = g[i-1] + g[i-2] + f[i-3];
}
